...
# In a real scenario, you would install this via pip
...
async def get_current_user(request: Request):
    ...
    if user_id:
        ...
        if user:
            ...
            return {"id": user['id'], "username": user['username'], "name": user['name'], "avatar_url": user.get('avatar_url')}
    return None
...
async def root(request: Request):
    ...
    return await inertia.render("Dashboard", {
        "user": user,
        "data": {"message": "Hello from FastAPI + Bun + Inertia!"}
    }, request)
...
async def tickets_index(request: Request):
    ...
    return await inertia.render("Tickets/Index", {
        "user": user,
        "tickets": tickets
    }, request)
...
async def show_ticket(ticket_id: int, request: Request):
    ...
    return await inertia.render("Tickets/Show", {
        "user": user,
        "ticket": ticket
    }, request)
