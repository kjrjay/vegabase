...
const pyreactDir = path.join(projectDir, ".pyreact");
...
async function generateEntryFiles() {
    // Ensure .pyreact directory exists
    if (!fs.existsSync(pyreactDir)) {
        fs.mkdirSync(pyreactDir, { recursive: true });
    }
...
`;
    ...
    fs.writeFileSync(path.join(pyreactDir, "pages.js"), pageMapCode);
    fs.writeFileSync(path.join(pyreactDir, "client.jsx"), clientEntry.trim());
    fs.writeFileSync(path.join(pyreactDir, "ssr.jsx"), ssrEntry.trim());
    fs.writeFileSync(path.join(pyreactDir, "ssr-server.jsx"), ssrServerEntry.trim());

    console.log("📝 Generated files in .pyreact/");
}

// Entry points in user's project
const entryClient = path.join(pyreactDir, "client.jsx");
const entrySSR = path.join(pyreactDir, "ssr.jsx");
const entrySSRServer = path.join(pyreactDir, "ssr-server.jsx");
...
async function dev() {
    ...
    const { default: tailwindcss } = await import(path.join(projectDir, 'node_modules', 'bun-plugin-tailwind'));
    ...
    const server = Bun.serve({
        ...
        websocket: {
            message() { },
            open(ws) { console.log("Browser connected to Hot Reload"); },
        },
    });
...
async function ssr() {
    ...
    if (!fs.existsSync(ssrPath)) {
        ...
        console.error("   Run 'pyreact build' first to create the production bundle.");
        process.exit(1);
    }
